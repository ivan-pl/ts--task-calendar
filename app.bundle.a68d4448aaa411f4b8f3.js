...
;// CONCATENATED MODULE: ./src/common/tasks/tasksSlice.ts
...
const addTask = task => async dispatch => {
  ...
  dispatch(tasksSlice_add({
    ...task,
    id
  }));
};
const selectTasks = state => state.tasks;
/* harmony default export */ const tasks_tasksSlice = (tasksSlice.reducer);
;// CONCATENATED MODULE: ./src/common/tasks/createAddForm.ts
...
`;
function createAddForm(date = new Date()) {
  const form = createElement("form", "new-task", null, TEMPLATE, "new-task");
  const dateInput = form.querySelector("#new-task__date");
  dateInput.value = format(date, "yyyy-MM-dd");
  const button = form.querySelector("#new-task__add");
  const descInput = form.querySelector("#new-task__description");
...
function handleClick() {
  const date = new Date(document.querySelector("#new-task__date").value);
  const description = document.querySelector("#new-task__description").value;
  const status = document.querySelector("#new-task__status").value;
  const tag = document.querySelector("#new-task__tag").value;
...
;// CONCATENATED MODULE: ./src/features/calendar/handleDay.ts


function handleDay(date) {
  setLayout(createAddForm(date));
  openModal();
}
/* harmony default export */ const calendar_handleDay = (handleDay);
...
function createWeek(week, currentMonth) {
  ...
  for (let i = 0; i < week.length; i++) {
    ...
    dayEl.addEventListener("click", () => calendar_handleDay(date));
...
;// CONCATENATED MODULE: ./src/common/localStorageController/storage.ts
class Storage {
  ...
  getItem(idOrKey) {
    const item = typeof idOrKey === "number" ? localStorage.getItem(this.keyById(idOrKey)) : localStorage.getItem(idOrKey);
    return item === null ? null : JSON.parse(item, (key, value) => {
      if (key === "date") {
  ...
  filterItems(propName, val) {
    let isEqual;
    switch (propName) {
      case "date":
        {
          const dateRange = val;
          isEqual = propVal => propVal >= dateRange.dateFrom && propVal <= dateRange.dateTo;
          break;
        }
      case "description":
        {
          const regexp = new RegExp(val, "i");
          isEqual = propVal => regexp.test(propVal);
          break;
        }
      default:
...
;// CONCATENATED MODULE: ./src/common/localStorageController/calendarController.ts

class CalendarController {
  static instanceIdGenerator = numberGenerator();
  constructor() {
  ...
  async getEventList(propName, propVal) {
...
const taskLocalStorage = new CalendarController();
